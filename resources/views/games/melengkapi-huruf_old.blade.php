    ...
    @push('scripts')
        <script>
            ...
            function createWordDisplay(kata, hurufHilang) {
                console.log(
                    "[createWordDisplay] called with kata:",
                    kata,
                    "hurufHilang:",
                    hurufHilang,
                )
                ...
                <span class="drop-zone inline-flex w-16 h-20 border-4 border-dashed border-orange-400 rounded-2xl mx-2 text-5xl font-black items-center justify-center text-center align-middle bg-white shadow-inner"
                    ...
                    _
                        ...
                        .join("")
                    console.log(
                        "[createWordDisplay] success, generated HTML length:",
                        result.length,
                    )
                    return result
                } catch (err) {
            ...
            function generateOptions(q) {
                ...
                try {
                    ...
                    console.log(
                        "[generateOptions] success, tiles generated:",
                        options.length,
                    )
                    return html
                } catch (err) {
            ...
            function renderQuestion() {
                ...
                try {
                    ...
                    if (!q) {
                        console.error(
                            "[renderQuestion] ERROR: no question found at index",
                            currentIndex,
                        )
                        return
                    }
                    ...
                    if (!container) {
                        console.error(
                            "[renderQuestion] ERROR: #gameContainer not found in DOM",
                        )
                        return
                    }
            ...
            `
                    ...
                    console.log(
                        "[renderQuestion] success, rendered question for word:",
                        q.kata,
                    )
                } catch (err) {
                ...
                try {
                    ...
                    if (audioSrc) {
                        ...
                        audio
                            ...
                            .catch((err) =>
                                console.error(
                                    "[drag] audio play FAILED for letter:",
                                    letter,
                                    err,
                                ),
                            )
            ...
            function drop(ev) {
                ...
                try {
                    ...
                    if (!dropZone) {
                        console.error(
                            "[drop] ERROR: no valid .drop-zone found for this drop target",
                        )
                        return
                    }
                    ...
                    if (data === correctLetter) {
                        ...
                        console.log(
                            "[drop] SUCCESS: correct letter placed, score now:",
                            score,
                        )
                        ...
                        const allFilled = zones.every(
                            (zone) => zone.textContent.trim() !== "_",
                        )
                        console.log(
                        ...
                        if (allFilled) {
                            setTimeout(() => {
                                if (currentIndex < questions.length - 1) {
                                    currentIndex++
                                    console.log(
                                        "[drop] advancing to next question, index:",
                                        currentIndex,
                                    )
                                    renderQuestion()
                                } else {
                                    console.log(
                                        "[drop] SUCCESS: all questions completed, final score:",
                                        score,
                                    )
                                    alert(
                                        `🎉 SELAMAT! Kamu menyelesaikan semua soal dengan skor ${score} poin!`,
                                    )
                                }
                            }, 800)
                        }
            ...
            function nextQuestion() {
                ...
                try {
                    if (currentIndex < questions.length - 1) {
                        currentIndex++
                        console.log(
                            "[nextQuestion] success, moving to index:",
                            currentIndex,
                        )
                        renderQuestion()
                    } else {
                        console.log(
                            "[nextQuestion] SUCCESS: reached end of questions, resetting. Final score was:",
                            score,
                        )
