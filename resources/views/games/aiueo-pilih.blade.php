<x-layout-game title="{{ $judul }}" halaman="{{ $halaman }}">
    <main class="relative z-10 mx-auto max-w-4xl p-6">
        ...
        <div
            class="mb-6 rounded-3xl bg-white p-8 text-center shadow-2xl"
            id="promptWrap"
        ></div>
        <div
            id="choicesWrap"
            class="mb-6 grid grid-cols-2 gap-4 md:grid-cols-3 md:gap-6"
        ></div>
    ...
    <script>
        ...
        function renderRound() {
            ...
            if (mode === "wordFromSound") {
                ...
                shuffle(round.groupItems).forEach((item) => {
                    ...
                    div.onclick = () =>
                        checkAnswer(item.id === round.item.id, div, () =>
                ...
                shuffle(round.groupDiphthongs).forEach((d) => {
                    ...
                    div.textContent = d
                    div.onclick = () =>
                        checkAnswer(d === round.diphthong, div, () =>
                            playAudio(round.item.audio),
                        )
                    choicesWrap.appendChild(div)
                })
            }
        ...
        function updateProgress() {
            document.getElementById("progressText").textContent =
                currentRound + " / " + roundOrder.length
            document.getElementById("progressBar").style.width =
                (currentRound / roundOrder.length) * 100 + "%"
        }
