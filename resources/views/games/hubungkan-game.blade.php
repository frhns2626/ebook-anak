<x-layout-game title="🔤 Belajar Huruf A-Z - Ebook Anak TK">
    <main class="relative z-10 p-6 max-w-4xl mx-auto">
        ...
        <div class="bg-yellow-100 border border-yellow-300 text-yellow-800 rounded-2xl p-4 mb-6 text-center font-medium">
            ...
            <span class="text-sm">Garis akan muncul menghubungkan huruf dan hewan.</span>
        </div>
        <div id="board" class="relative bg-white rounded-3xl p-6 shadow-xl">
            ...
            <div class="grid grid-cols-2 gap-20 md:gap-32 relative" style="z-index:2">
                <div>
                    <h3 class="text-center text-xl font-bold text-gray-700 mb-4">Huruf</h3>
                    <div id="letters" class="space-y-4"></div>
                </div>
                <div>
                    <h3 class="text-center text-xl font-bold text-gray-700 mb-4">Hewan</h3>
        ...
        <div class="flex justify-center gap-4 mt-8">
            <button id="resetBtn"
                    class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-4 rounded-full font-bold transition-all">
                🔄 Ulangi
            </button>
        </div>
        <div id="successModal" class="hidden fixed inset-0 bg-black/70 flex items-center justify-center z-50">
            <div class="bg-white rounded-3xl p-10 text-center max-w-md mx-4">
                ...
                <p class="text-xl text-gray-600 mb-6">Semua huruf sudah cocok dengan benar!</p>
    ...
    <script>
        ...
        function renderBoard() {
            ...
            shuffle([...items]).forEach(item => {
                ...
                div.className = 'item-box border-4 border-blue-200 rounded-2xl p-5 text-center cursor-pointer transition-all shadow-md text-5xl font-black flex items-center justify-center h-24 select-none';
            ...
            shuffle([...items]).forEach(item => {
                ...
                div.dataset.letter = item.letter;
                div.dataset.id = item.id;
                div.className = 'item-box border-4 border-purple-200 rounded-2xl p-5 cursor-pointer transition-all shadow-md flex items-center justify-center gap-4 h-24';
                div.innerHTML = `<span class="text-7xl flex-shrink-0">${item.emoji}</span><div class="font-bold text-xl">${item.name}</div>`;
