...
>
    ...
    <div
        id="gameContainer"
        class="relative my-auto flex h-full w-full flex-col items-center justify-between select-none"
    >
        <canvas
            id="lineCanvas"
        ...
        <div class="z-10 mt-2 grid w-full grid-cols-4 gap-3 px-2 sm:gap-4">
            @foreach (collect($items)->shuffle() as $item)
                <div class="flex flex-col items-center space-y-2">
                    <div
                        class="item-card h-16 w-full text-3xl font-extrabold text-black sm:h-20 sm:text-4xl"
                    >
                        {{ $item['letter'] }}
                    </div>
                    <div
                        class="connect-dot"
                        data-type="top"
                        data-id="{{ $item['letter'][0] }}"
                        data-match="{{ strtolower($item['id']) }}"
                    ></div>
                </div>
            @endforeach
        </div>
        <div class="min-h-[120px] flex-1"></div>
        <div class="z-10 mb-2 grid w-full grid-cols-5 gap-4 px-2">
            @foreach (collect($items)->shuffle() as $item)
                @php
        ...
        <script>
            document.addEventListener("DOMContentLoaded", () => {
                ...
                function startDrag(e) {
                    ...
                    if (
                        connections.some((c) => c.from === dot || c.to === dot)
                    ) {
                        return
                    }
