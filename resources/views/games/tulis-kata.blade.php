<x-layout-game>
    <main class="relative z-10 p-6 max-w-4xl mx-auto">
        ...
        <div id="itemDisplay" class="bg-white rounded-3xl p-8 shadow-2xl text-center mb-6 hidden">
            ...
            <div class="flex justify-center gap-3 flex-wrap mb-2" id="syllableChips"></div>
            <p class="text-gray-400 text-xs mb-6">Baca suku katanya dengan nyaring, lalu ketik kata utuhnya di bawah</p>
            <input type="text" id="writeInput" autocomplete="off"
                   class="text-center text-[1.8rem] font-black text-gray-800 border-b-4 border-gray-300 focus:border-purple-400 outline-none w-full max-w-xs mx-auto mb-2 pb-1"
                   placeholder="Tulis kembali...">
            <p id="writeFeedback" class="h-6 mb-4 font-bold text-sm"></p>
            <p class="text-gray-500 text-[1.1rem] mb-6" id="itemHint">Bola adalah alat main bulat!</p>
            <div class="flex justify-center gap-4 flex-wrap">
                ...
                <button onclick="checkWrite()" id="checkBtn"
                        class="bg-gradient-to-r from-purple-400 to-purple-500 text-white px-6 py-3 rounded-full font-bold shadow-lg hover:scale-105 transition-all">✅ Periksa
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        function renderSyllableChips(item) {
            ...
            item.syllables.forEach(s => {
                ...
                chip.textContent = s;
                chip.className = 'bg-gradient-to-r from-orange-400 to-orange-500 text-white px-4 py-2 rounded-full font-black text-xl shadow-md';
