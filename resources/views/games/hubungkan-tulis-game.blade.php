...
>
    ...
    <div
        id="gameContainer"
        class="relative my-auto flex h-full w-full flex-col items-center justify-between px-2 select-none"
    >
        ...
        <!-- Judul Atas -->
        <div class="z-10 mt-1 mb-2 text-center">
            <h1
                class="title-text px-2 text-lg leading-tight font-extrabold tracking-wide sm:text-xl md:text-2xl"
            >
        ...
        <div
            class="z-10 my-auto grid w-full max-w-xl grid-cols-3 gap-3 sm:gap-6"
        >
                ...
                <div class="flex flex-col items-center space-y-2">
                    <div class="item-card" data-audio="{{ $item['audio'] }}">
                        <img
                            src="{{ $item['emoji'] }}"
                            alt="{{ $word }}"
                            class="pointer-events-none h-16 object-contain sm:h-20"
                        />
                        <span
                            class="mt-1 text-xl font-black text-black sm:text-2xl"
                            >{{ $word }}</span
                        >
                    </div>
                    <div
        ...
        <!-- Space Tengah -->
        <div class="min-h-[100px] w-full flex-1"></div>

        <!-- Baris Bawah: Titik Hubung + Kata Tujuan + Kotak Tulis Ulang -->
        <div class="z-10 mb-2 grid w-full max-w-xl grid-cols-3 gap-2 sm:gap-4">
            @foreach ($bottomItems as $item)
                ...
                <div class="flex flex-col items-center space-y-2">
                    ...
                    ></div>
                    <span
                        class="text-xl font-black tracking-widest text-black"
                        >{{ implode(' ', str_split($word)) }}</span
                    >
                    <div
            ...
            document.addEventListener("DOMContentLoaded", () => {
                ...
                document.querySelectorAll(".item-card").forEach((card) => {
                    card.addEventListener("click", () => {
                        ...
                        wordAudioPlayer.addEventListener("ended", () => {
                            isWordAudioPlaying = false
                        })
                        wordAudioPlayer.addEventListener("error", () => {
                            isWordAudioPlaying = false
                        })
                        wordAudioPlayer.play().catch(() => {
                            isWordAudioPlaying = false
                        })
                ...
                function evaluateConnection(startDot, endDot) {
                    ...
                    if (
                        !endDot ||
                        startDot === endDot ||
                        startDot.dataset.type === endDot.dataset.type
                    ) {
                        drawLines()
                        return
                    }
                    ...
                    const topDot =
                        startDot.dataset.type === "top" ? startDot : endDot
                    const bottomDot =
                        startDot.dataset.type === "bottom" ? startDot : endDot
                    ...
                    if (topDot.dataset.match === bottomDot.dataset.id) {
                        ...
                        if (typeof showFlashMessage === "function") {
                            showFlashMessage(
                                "success",
                                "Hebat! Garis terhubung dengan benar!",
                            )
                        }

                        const group = document.querySelector(
                            `[data-word="${bottomDot.dataset.id}"]`,
                        )
                        if (group) {
                        ...
                        if (typeof showFlashMessage === "function") {
                            showFlashMessage(
                                "error",
                                "Salah, coba hubungkan ke kata yang cocok!",
                            )
                        }
                    }
                }
                ...
                function endDrag(e) {
                    ...
                    const targetDot = targetElement
                        ? targetElement.closest(".connect-dot")
                        : null
                ...
                charInputs.forEach((input) => {
                    input.addEventListener("input", (e) => {
                        ...
                        if (val === targetChar) {
                            ...
                            if (
                                nextInput &&
                                nextInput.classList.contains("char-input")
                            ) {
                                nextInput.focus()
                            }
                            ...
                            const allInputs =
                                group.querySelectorAll(".char-input")
                            const allCorrect = Array.from(allInputs).every(
                                (i) => i.classList.contains("correct"),
                            )

                            if (allCorrect) {
                                const wordAudioSrc = group.dataset.audio
                                if (wordAudioSrc) {
                                    const wordDoneAudio = new Audio(
                                        wordAudioSrc,
                                    )
                    ...
                    input.addEventListener("keydown", (e) => {
                        if (e.key === "Backspace" && input.value === "") {
                            ...
                            if (
                                prevInput &&
                                prevInput.classList.contains("char-input")
                            ) {
                                prevInput.focus()
                            }
                        }
