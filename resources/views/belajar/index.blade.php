...
<body class="bg-gradient-to-b from-yellow-50 via-emerald-50 to-green-50 min-h-screen overflow-x-hidden relative">
    {{-- Background overlay --}}
    <div class="fixed inset-0 pointer-events-none z-0" style="background:radial-gradient(ellipse at 20% 20%, rgba(255,248,225,0.8) 0%, transparent 50%), radial-gradient(ellipse at 80% 80%, rgba(200,245,219,0.6) 0%, transparent 50%), radial-gradient(ellipse at 50% 50%, rgba(255,255,255,0.3) 0%, transparent 70%)"></div>
    ...
    {{-- Floating animals --}}
    <div class="fixed z-10 pointer-events-none animate-float-gentle text-[3.5rem] hidden md:block" style="bottom:18%;left:3%;animation-duration:5s">🐰</div>
    <div class="fixed z-10 pointer-events-none animate-float-gentle text-[3.5rem]" style="bottom:22%;right:4%">🦊</div>
    <div class="fixed z-10 pointer-events-none animate-float-gentle text-[3.5rem] hidden md:block" style="top:20%;left:5%">🐻</div>
    <div class="fixed z-10 pointer-events-none animate-float-gentle text-[3.5rem]" style="top:22%;right:6%">🐱</div>

    {{-- Butterflies --}}
    <div class="fixed z-10 pointer-events-none animate-float-side text-[3rem] hidden md:block" style="top:35%;left:4%;animation-delay:3s">🦋</div>
    <div class="fixed z-10 pointer-events-none animate-float-side text-[3rem] hidden md:block" style="top:40%;right:5%">🦋</div>

    {{-- Decorative flowers --}}
    <div class="fixed z-10 pointer-events-none text-[2.5rem] hidden md:block" style="bottom:15%;left:4%">🌸</div>
    <div class="fixed z-10 pointer-events-none text-[2.5rem]" style="bottom:20%;right:5%">🌺</div>

    {{-- Main content --}}
    <main class="relative z-20 max-w-4xl mx-auto px-8 py-10">
        {{-- Header --}}
        {{-- Kembali Button --}}
        ...
        <header class="text-center mb-12 animate-bounce-subtle">
            <h1 class="text-[3rem] md:text-[3.5rem] font-black leading-tight mb-4 bg-gradient-to-r from-violet-600 via-indigo-500 to-blue-500 bg-clip-text text-transparent drop-shadow-lg">
                🎓 Belajar Yuk!
            </h1>
            <p class="text-gray-600 text-[1.2rem] md:text-[1.4rem] font-bold">
...
[
            ...
            <article class="bg-white rounded-3xl p-6 shadow-2xl border-4 border-white relative overflow-hidden transition-all duration-300 hover:-translate-y-3 hover:shadow-2xl hover:border-{{ $module['accent'] }}-100 group">
                ...
                <!-- Top Gradient -->
                <div class="absolute top-0 left-0 right-0 h-2 rounded-t-3xl"
                ...
                <div class="w-full h-28 rounded-xl flex items-center justify-center mb-4 bg-gradient-to-br from-{{ $module['accent'] }}-100 to-{{ $module['accent'] }}-200">
                    <span class="text-[4rem] relative z-10 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3"
                ...
                <div class="text-center">
                    <h2 class="text-[1.3rem] font-black text-gray-800 mb-2">
                        {{ $module['title'] }}
                    </h2>
                    <p class="text-gray-500 text-[0.9rem] mb-5 font-semibold leading-relaxed">
                        {{ $module['subtitle'] }}
                    </p>

                    <a href="{{ route($module['route']) }}"
                        class="inline-flex items-center justify-center gap-2 w-full px-4 py-3 text-[1.1rem] font-extrabold text-white bg-gradient-to-r from-orange-500 via-orange-400 to-amber-400 rounded-xl shadow-lg hover:-translate-y-1 hover:scale-105 transition-all"
                        style="box-shadow: 0 8px 25px rgba(249,115,22,0.4)">
                        <span class="text-[1.2rem]">📚</span> Buka
                    </a>
                </div>
            </article>
            @endforeach
        ...
        {{-- Progress Section --}}
        <section class="bg-white rounded-3xl p-8 shadow-xl border-4 border-white text-center">
            <p class="text-[1.2rem] font-extrabold text-gray-700 mb-4">
                <span class="text-[1.3rem]">🌟</span>
                Ayo mulai belajar! Pilih modul di atas untuk memulai!
            </p>
            <a href="{{ route('belajar.halaman-1') }}" class="inline-flex items-center gap-3 px-10 py-5 text-[1.25rem] font-extrabold text-white bg-gradient-to-r from-violet-500 via-purple-400 to-fuchsia-400 rounded-2xl shadow-xl hover:-translate-y-1 hover:scale-102 transition-all" style="box-shadow:0 10px 30px rgba(139,92,246,0.4)">
                <span class="text-[1.3rem]">🚀</span>
    ...
    <svg class="fixed bottom-0 left-0 w-full h-30 z-0 pointer-events-none" viewBox="0 0 1440 120" fill="none" preserveAspectRatio="none">
